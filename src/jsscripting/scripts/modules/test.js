...
this.onStart = function() {
    this.mEntity.setUserInteractionEnabled(true);
    var entMgr = this.mEntity.getEntityManager();
    scoreLabel = new hoolai.gui.Label(hoolai.gui.GUIManager.getRootWidget(), {x:260, y:50, width:100, height:50});
    scoreLabel.textAlign = hoolai.TextAlignmentType.HLeft;
    scoreLabel.fontSize = 18;
    scoreLabel.text = "score\n\n0";
    scoreLabel.textColor = {r:1, b:1, g:1, a:1};
...
//    mEntity.addChild(scorenum);
    textures[0] = hoolai.Texture.getTexture("blue.png");
    textures[1] = hoolai.Texture.getTexture("cyan.png");
    textures[2] = hoolai.Texture.getTexture("green.png");
    textures[3] = hoolai.Texture.getTexture("magenta.png");
    textures[4] = hoolai.Texture.getTexture("orange.png");
    textures[5] = hoolai.Texture.getTexture("red.png");
    textures[6] = hoolai.Texture.getTexture("yellow.png");
    
    for (i = 0; i < 7; ++i) { // retain textures
        var entity = entMgr.createEntity("texture"+i, "HLTransform2DComponent", "HLSpriteComponent");
        entity.texture = textures[i];
    }
...
//            mEntity.getEntityManager().getEntity("score").text = totalScore+"";
    ...
    if (dirty) {
        ...
        for (i = 0; i < total_columns*total_rows; i++) {
            map[i] = unitsMap[i];
            if(map[i] == 1) {
                var entity = this.mEntity.getEntityManager().getEntity("block"+i);
        ...
        for (i = 0; i < 4; i++) {
            for (j = 0; j < 4; j++) {
                if (units[nowUnitNum][i*4+j] == 1) {
                    ...
                    map[index] = 1;
                    var entity = this.mEntity.getEntityManager().getEntity("block"+index);
